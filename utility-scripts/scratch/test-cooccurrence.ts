...
async function run() {
  ...
  try {
    console.log("=== MINING BRAND ASSORTMENT CO-OCCURRENCE ===");
    
    // Find category pairs that share a high number of active, vector-embedded brands
    const query = `
      WITH active_brands AS (
        SELECT id FROM brands_search_mv WHERE embedding IS NOT NULL
      ),
      active_cats AS (
        SELECT id, name FROM product_categories_search_mv WHERE embedding IS NOT NULL
      ),
      category_pairs AS (
        SELECT 
